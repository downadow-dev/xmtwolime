...
def compile_obj(obj, root=False, flt=False, uns=False):
    ...
    try:
        ...
        # присваивание
            ...
            return compile_obj(obj.lvalue, uns=is_uns(obj))[:-2] + ' dup ' + ('dup' if not root else '') + ' . ' + compile_obj(obj.rvalue) + ' ' + obj.op[0].replace('/', ('/' if not is_uns(obj) else 'u/')).replace('%', ('mod' if not is_uns(obj) else 'umod')).replace('&', 'and').replace('<', 'lsh').replace('>', 'rsh') + ' swap = ' + ('.' if not root else '')
        # сложение, вычитание и др.
        elif type(obj) == BinaryOp and obj.op in '-+/*^|%':
            return compile_obj(obj.left) + ' ' + compile_obj(obj.right) + ' ' + obj.op.replace('/', ('/' if not is_uns(obj) else 'u/')).replace('%', ('mod' if not is_uns(obj) else 'umod'))
        elif type(obj) == BinaryOp and obj.op == '<<':
