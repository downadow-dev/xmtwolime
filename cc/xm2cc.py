...
def compile_obj(obj, root=False, flt=False):
    ...
    try:
        ...
        # присваивание
            ...
            return compile_obj(obj.lvalue)[:-2] + ' ' + ('dup' if not root else '') + ' ' + compile_obj(obj.rvalue) + ' swap = ' + ('.' if not root else '')
        elif type(obj) == Assignment and obj.op[0] in '+-/*^%|&<>' and obj.op.endswith('='):
            return compile_obj(obj.lvalue)[:-2] + ' dup ' + ('dup' if not root else '') + ' . ' + compile_obj(obj.rvalue) + ' ' + obj.op[0].replace('%', 'mod').replace('&', 'and').replace('<', 'lsh').replace('>', 'rsh') + ' swap = ' + ('.' if not root else '')
...
def compile_for_xmtwolime(prog_name, tree, outfile):
    ...
    asm('mov %R_FA_15%, %OUT_ST%')  # сброс указателя вывода
